...
public:
  ImageScriptObject() {
    ...
    addMethod("drawLine", &ImageScriptObject::drawLine)
      .doc("draws a line on the image.")
      .docArg("x1", "integer").docArg("y1", "integer")
      .docArg("x2", "integer").docArg("y2", "integer")
      .docArg("color", "a 32-bit color in 8888 RGBA format.");

    addMethod("drawRect", &ImageScriptObject::drawRect)
      .doc("draws a rectangle outline on the image.")
      .docArg("x", "integer").docArg("y", "integer")
      .docArg("w", "integer").docArg("h", "integer")
      .docArg("color", "a 32-bit color in 8888 RGBA format.");

    addMethod("drawEllipse", &ImageScriptObject::drawEllipse)
      .doc("draws an ellipse outline on the image.")
      .docArg("x", "integer").docArg("y", "integer")
      .docArg("w", "integer").docArg("h", "integer")
      .docArg("color", "a 32-bit color in 8888 RGBA format.");

    addFunction("clone", [this]() -> script::ScriptObject* {
      auto copy = doc::Image::createCopy(img());
      return getEngine()->getScriptObject(copy.release());
    }).doc("returns a copy of this image.")
      .docReturns("A new Image object with the same pixel data.");
  ...
  void drawLine(int x1, int y1, int x2, int y2, int color) {
    doc::draw_line(img(), x1, y1, x2, y2, color);
  }

  void drawRect(int x, int y, int w, int h, int color) {
    doc::draw_rect(img(), x, y, x+w-1, y+h-1, color);
  }

  void drawEllipse(int x, int y, int w, int h, int color) {
    doc::draw_ellipse(img(), x, y, x+w-1, y+h-1, color);
  }
};
