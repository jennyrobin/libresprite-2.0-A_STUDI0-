...
public:
  LayerScriptObject() {
    ...
    addProperty("isLocked",
      ...
      .doc("read+write. Gets/sets whether the layer is locked (not editable).");

    addProperty("opacity",
                [this]{
                  if (layer()->isImage())
                    return static_cast<int>(static_cast<doc::LayerImage*>(layer())->opacity());
                  return 255;
                },
                [this](int opacity){
                  if (layer()->isImage())
      ...
      .doc("read+write. Gets/sets the layer opacity (0-255).");
