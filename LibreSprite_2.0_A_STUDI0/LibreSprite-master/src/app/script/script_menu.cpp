...
void scanFolder(const std::string& scriptsDir, Command* cmd_run_script, Menu* parent) {
  ...
  for (auto child : list) {
    ...
    // Register this script as a Key in the keyboard shortcuts system
    // so it appears in Edit > Keyboard Shortcuts and can be assigned a hotkey
    if (!isFolder && cmd_run_script) {
      Key* key = KeyboardShortcuts::instance()->command(
        CommandId::RunScript, params, KeyContext::Any);
      if (key) {
        // Set the display label to the script's display name
        menuitem->setKey(key);
      }
    }
