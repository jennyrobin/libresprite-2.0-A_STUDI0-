...
public:
  ...
  DukEngine() :
                             ...
                             &on_fatal_handler))
    {
      ...
      duk_peval_string(m_handle,
        "(function(global){"
        "if (typeof global.Color !== 'function') {"
        "global.Color = function(args) {"
        "args = args || {};"
        "var r = args.red; if (r === undefined) r = args.r || 0;"
        "var g = args.green; if (g === undefined) g = args.g || 0;"
        "var b = args.blue; if (b === undefined) b = args.b || 0;"
        "var a = args.alpha; if (a === undefined) a = (args.a === undefined ? 255 : args.a);"
        "return { red:r, green:g, blue:b, alpha:a, r:r, g:g, b:b, a:a };"
        "};"
        "}"
        "})(this);");
      duk_pop(m_handle);
...
public:
  static Value getValue(duk_context* ctx, int id) {
    ...
    if (type == DUK_TYPE_NUMBER) {
      ...
      return bool(duk_get_boolean(ctx, id));
    } else if (type == DUK_TYPE_NULL) {
      return {};
    } else if (type == DUK_TYPE_OBJECT) {
      if (duk_is_callable(ctx, id)) {
        void* heapptr = duk_get_heapptr(ctx, id);
        return Value::Callable([ctx, heapptr](const std::vector<Value>& args) -> Value {
          duk_push_heapptr(ctx, heapptr);
          for (auto& arg : args) {
            if (!returnValue(ctx, arg))
              duk_push_undefined(ctx);
          }
          if (duk_pcall(ctx, args.size()) != 0) {
            std::string err = duk_safe_to_string(ctx, -1);
            std::cout << "Error: [" << err << "]" << std::endl;
            duk_pop(ctx);
            return {};
          }
          Value ret = DukScriptObject::getValue(ctx, -1);
          duk_pop(ctx);
          return ret;
        });
      }
      ...
      auto& obj = objptr[0];
      int objIndex = duk_normalize_index(ctx, id);
      if (duk_is_array(ctx, objIndex))
        obj["length"] = int(duk_get_length(ctx, objIndex));
...
static duk_ret_t returnValue(duk_context* ctx, const Value& value) {
    ...
    case Value::Type::MAP: {
        auto map = static_cast<Value::Map::data_t*>(value);
        if (!map) {
          duk_push_null(ctx);
          break;
        }

        bool arrayLike = map->find("length") != map->end();
        if (arrayLike)
          duk_push_array(ctx);
        else
          duk_push_object(ctx);
        ...
        for (auto& entry : *map) {
          if (!returnValue(ctx, entry.second))
            duk_push_undefined(ctx);

          if (arrayLike && entry.first != "length") {
            char* end = nullptr;
            long index = std::strtol(entry.first.c_str(), &end, 10);
