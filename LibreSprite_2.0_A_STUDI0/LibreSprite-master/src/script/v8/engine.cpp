...
static v8::Local<v8::Value> returnValue(v8::Isolate* isolate, const Value& value) {
    ...
    case Value::Type::MAP: {
        ...
        if (arrayLike)
            object = v8::Array::New(isolate);
        else
            object = v8::Object::New(isolate);

        for (auto& entry : *map) {
            auto propValue = returnValue(isolate, entry.second);
            if (propValue.IsEmpty())
                propValue = v8::Undefined(isolate);

            if (arrayLike && entry.first != "length") {
                char* end = nullptr;
                long index = std::strtol(entry.first.c_str(), &end, 10);
                if (end && *end == '\0' && index >= 0) {
                    Check(object->Set(context, uint32_t(index), propValue));
                    continue;
                }
            }
