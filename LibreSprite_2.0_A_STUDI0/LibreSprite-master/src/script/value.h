...
namespace script {
  ...
  public:
	...
	data.map_v = new Map {*other.data.map_v};
        ...
        data.callable_v = new Callable {*other.data.callable_v};
      } else {
        data = other.data;
      }
      return *this;
...
// INT
    ...
    operator int () const {
      if (type == Type::BOOL) return data.bool_v ? 1 : 0;
...
// DOUBLE
    ...
    operator double () const {
      if (type == Type::BOOL) return data.bool_v ? 1.0 : 0.0;
...
// STRING
    ...
    std::string str() const {
      if (type == Type::BOOL) return data.bool_v ? "true" : "false";
...
// CALLABLE
    Value(const Callable& callable) { *this = callable; }
    Value(Callable&& callable) { *this = std::move(callable); }

    Value& operator = (const Callable& callable) {
      makeUndefined();
      type = Type::CALLABLE;
      data.callable_v = new Callable(callable);
      return *this;
    }

    Value& operator = (Callable&& callable) {
      makeUndefined();
      type = Type::CALLABLE;
      data.callable_v = new Callable(std::move(callable));
      return *this;
    }

    Value call(const std::vector<Value>& args = {}) const {
      if (type == Type::CALLABLE && data.callable_v)
        return (*data.callable_v)(args);
      return {};
    }
